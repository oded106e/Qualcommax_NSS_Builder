...
function summarise(list){if(!list.length)return null;return list.length>1?'%s +%d'.format(list[0],list.length-1):list[0];}
function fmtAddrList(value){return summarise((value||'').split(',').map(function(a){return a.trim();}).filter(function(a){return a.length>0;}));}
return baseclass.extend({formatDuration:formatDuration,fmtBytes:fmtBytes,fmtAddr:fmtAddr,fmtAddrList:fmtAddrList,});
